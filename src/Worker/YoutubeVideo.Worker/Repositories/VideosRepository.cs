    ...
    public class VideosRepository
    {
        ...
        public static void CriarTabelaSQlite()
        {
            try
            {
                ...
                {
                    cmd.CommandText = "CREATE TABLE Videos (Id INT IDENTITY(1,1) PRIMARY KEY, Title NVARCHAR(255) NOT NULL, " +
        ...
        public static void Add(Video video)
        {
            try
            {
                using (var cmd = DbConnection().CreateCommand())
                {
                    cmd.CommandText = "INSERT INTO Videos (Title, Description, ChannelTitle, Author, Duration, PublishedAt, IsDeleted, Link) " +
                      "VALUES (@Title, @Description, @ChannelTitle, @Author, @Duration, @PublishedAt, @IsDeleted, @Link);";
